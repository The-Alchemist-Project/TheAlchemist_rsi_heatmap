...
ls_coin_ls = ['BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'SUIUSDT', 'ADAUSDT', 'DOGEUSDT', 'MATICUSDT', 'SOLUSDT', 'DOTUSDT', 'TRXUSDT', 'LTCUSDT', 'AVAXUSDT', 'LINKUSDT', 'ATOMUSDT', 'UNIUSDT', 'XRMUSDT', 'OKBUSDT', 'ETCUSDT', 'TONUSDT', 'XLMUSDT', 'SUIUSDT']
...
class Crawler(object):
    ...
    def get_coin_info(self):
        query_str = "select symbol from stock_price"
    ...
    def get_last_update_timestamp(self, symbol):
        query_str = f'select max(open_time) from stock_price where symbol = "{symbol}"'
        fdf = pd.read_sql(query_str, con=engine)
        if fdf.values.flatten()[0] == None:
            return glob_start_timestamp
    ...
    def run_crawl_stock_price(self):
        ...
        for symbol in symbols:
            ...
            # write to db
            klines_df.to_sql('stock_price', con=engine, if_exists='append', chunksize=500, index=False)
